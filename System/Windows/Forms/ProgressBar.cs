...
namespace System.Windows.Forms
{
    ...
    public partial class ProgressBar : Control
    {
            ...
            set
            {
                if (_maximum != value)
                {
                    ...
                    if (IsHandleCreated)
                    {
                        ...
                        if (layoutPerformed)
                        {
                            Page.RunScript($"document.getElementById(\"{WebviewIdentifier}\").setAttribute('data-max', '{_maximum}');");
                        }
                        UpdatePos();
                    }
                }
        ...
        public int Value
        {
            ...
            set
            {
                if (_value != value)
                {
                    ...
                    {
                        throw new ArgumentOutOfRangeException(nameof(value), value, string.Format("InvalidBoundArgument", nameof(Value), value, nameof(Minimum), nameof(Maximum)));
                    }
        ...
        //internal override unsafe void CreateHandle()
        //{
        //    if (!RecreatingHandle)
        //    {
        ...
        public void PerformStep()
        {
            if (Style == ProgressBarStyle.Marquee)
            {
                throw new InvalidOperationException("ProgressBarPerformStepMarqueeException");
            }
        ...
        private void UpdatePos()
        {
            if (IsHandleCreated)
            {
                if (layoutPerformed)
                {
                    Page.RunScript($"document.getElementById(\"{WebviewIdentifier}\").setAttribute('data-value', '{_value}');");
                }
                //PInvokeCore.SendMessage(this, PInvoke.PBM_SETPOS, (WPARAM)_value);
            }
        }
        ...
        public async override void PerformLayout()
        {
            ...
            await Page.Add(Parent.WebviewIdentifier, "innerHTML", $"'<div id=\"{WebviewIdentifier}\" class=\"progress\" data-value=\"{Value}\" data-min=\"{Minimum}\" data-max=\"{Maximum}\" style=\"{style}\"><div class=\"progress-value\"></div></div>';");
