    ...
    internal class NativeMethods
    {
        ...
        */
        ...
        public static class Util
        {
            ...
            public static int lstrlen(string managedString)
            {
                if (string.IsNullOrEmpty(managedString))
                    return 0;

                // Get encoding for ANSI with EncoderExceptionFallback to detect characters not representable in ANSI
                Encoding ansiEncoding = Encoding.GetEncoding(
                    1252,
                ...
                try
                {
                    // Try encoding to ANSI — if all characters are representable, get byte count
                    byte[] ansiBytes = ansiEncoding.GetBytes(managedString);
                    return ansiBytes.Length; // ANSI byte count (lstrlenA semantics)
                }
                catch (EncoderFallbackException)
                {
                    // Some characters cannot be encoded as ANSI, fallback to Unicode length
                    return managedString.Length; // Unicode char count (lstrlenW semantics)
                }
            }
